...
9. **Fatal burden (lines ~1095–1310).** Applies `fatal_sample`/`nh_fatal_sample` (hospitalised/non-hospitalised fatality rates by age band) to the step2 output to get deaths, then builds `p_fatal_burden` the same way as the hospitalisation plots. See the "known data-quality issue" callout above — the `fatal_sample` values used here still come from the cruder of two formulas defined in `lhs_samples.R`.
...
- Fix the `fatal_sample` double-definition in `lhs_samples.R` (out of this script's scope, but see the callout above).
