...
|---|---|---|
| `le_sample`, `lhs_sample_young`, `lhs_old` | `lhs_samples.R` (root) | LHS Monte Carlo draws, unchanged. |
| `hosp_sample`, `fatal_sample` (saved as `hosp_sample_v2.RData`, `fatal_sample_v2.RData`) | `lhs_updated.R` (root, new as of 2026-07-15) | Re-derived from raw count data (`MainData/hosp_chikvna.RDS`, `MainData/death_chikvna.RDS`) using exact binomial variance (p(1-p)/n) per 10-year age band — supersedes `lhs_samples.R`'s hardcoded versions for these two. `open_data.R` now loads the `_v2` files, not the originals. |
| `nh_fatal_sample` (saved as `nh_fatal_sample_v2.RData`) | `lhs_updated.R` | Still uses `lhs_samples.R`'s old CV-carryover approach applied to `MainData/chikv_fatal_hosp_rate.RData`'s point estimates — no non-hospitalised death count data exists yet to do the same exact-variance treatment as hosp/fatal. **`lhs_samples.R`'s original `fatal_sample` double-definition bug (lines ~346/~381) is now moot for the live pipeline** since `open_data.R` no longer loads `lhs_samples.R`'s output for `fatal_sample` — but the bug still exists in `lhs_samples.R` itself if anyone runs that script directly. |
...
flowchart TD
    LHS["lhs_samples.R"]
    LHSU["lhs_updated.R\n(new 2026-07-15)"]
    HOSPRAW["MainData/hosp_chikvna.RDS"]
    DEATHRAW["MainData/death_chikvna.RDS"]
    CHIKVRATE["MainData/chikv_fatal_hosp_rate.RData\n(hosp, fatal, nh_fatal point estimates)"]
